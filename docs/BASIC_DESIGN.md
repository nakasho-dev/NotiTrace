...
| 7 | **JSON ビューア画面** | `detail/{id}/json` | 通知 1 件の全フィールドを整形 JSON でシンタックスハイライト表示し、クリップボードへのコピーと JSON ファイル共有を提供する |
...
- **目的**: 通知の全フィールドを JSON 形式で一覧し、開発者やパワーユーザーが生データを確認・コピー・共有できるようにする
- **主要要素**:
  - TopAppBar: タイトル「JSON 生データ」+ 戻るボタン + Copy ボタン（`ContentCopy` アイコン）+ Share ボタン（`Share` アイコン）
  ...
  - Share 機能: TopAppBar の Share ボタンタップで JSON を UTF-8 の `.json` ファイルとして `cache/shared_json/` に一時出力し、`FileProvider` 経由で共有シートを起動する。Quick Share などのファイル共有アプリを利用可能
  - エラーハンドリング: 共有ファイル作成失敗時、共有アプリ未検出時はいずれも `Snackbar` で失敗理由を表示
...
- **設計ポイント**: 外部ライブラリを使用せず、簡易パーサーで JSON シンタックスハイライトを実装。ダーク/ライト両テーマで視認性を確保しつつ、共有は Android 標準の共有シートと `FileProvider` で完結させる
...
| F-13 | JSON 生データ表示 | 推奨 | 通知詳細画面から遷移し、`StatusBarNotification` から直接ダンプした Android OS 由来の生データを整形 JSON でシンタックスハイライト表示。アプリ独自の加工データは含まない。クリップボードコピーと JSON ファイル共有に対応 |
...
│   │   │   ├── DetailScreen.kt
│   │   │   ├── DetailViewModel.kt
│   │   │   └── JsonViewerScreen.kt     # JSON 生データ表示（シンタックスハイライト + コピー/共有）
│   │   ├── search/
│   │   │   ├── SearchScreen.kt
│   │   │   └── SearchViewModel.kt
│   │   ├── tag/
